		...
		<div className="columns is-centered">
			{
				footer
					.map(section => (
						<div className="column is-narrow content" key={'footer-section-' + section.section}>
							<p className="is-size-6">{section.section}</p>
							{
								section.links.map(link => 
									<><a className="footer-link is-text" key={'footer-link-' + link.name} href={link.link}>{link.name}</a><br/></>
								)
							}
						</div>
					))
